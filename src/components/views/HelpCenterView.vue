  ...
  <div class="max-w-6xl mx-auto">
    ...
    <div class="mb-8">
      <div class="flex items-center gap-3 mb-1">
        ...
        <h1 class="text-[24px] font-extrabold tracking-tight text-[#18181B]">
          Centro de Ayuda
        </h1>
      </div>
      <p class="text-[14px] text-[#71717A] font-medium ml-[40px]">
        Encuentra guías, tutoriales y respuestas a las preguntas más frecuentes sobre Contex360.
      </p>
    </div>
    ...
    <div class="relative mb-8">
      <div class="flex items-center gap-3 border border-[#E4E4E7] rounded-[14px] px-4 py-3.5 bg-white focus-within:border-[#2563EB] focus-within:shadow-[0_0_0_3px_rgba(37,99,235,0.1)] transition-all shadow-sm">
        ...
        <kbd
          v-if="!searchQuery"
          class="hidden sm:inline-block text-[10px] font-mono text-[#A1A1AA] border border-[#E4E4E7] rounded px-1.5 py-0.5 bg-[#FAFAFA]"
        >⌘K</kbd>
      </div>
    </div>
    ...
    >
      <button
        class="flex items-center gap-1.5 text-[13px] font-semibold text-[#71717A] hover:text-[#18181B] mb-5 transition-colors"
        @click="closeArticle"
      >
      ...
      <div class="bg-white border border-[#E4E4E7] rounded-[16px] p-6 sm:p-8 shadow-sm">
        <div class="flex items-start gap-4 mb-6">
          ...
          <div>
            <h2 class="text-[20px] font-extrabold tracking-tight text-[#18181B] mb-1">
        ...
        <p class="text-[14px] text-[#71717A] leading-relaxed mb-6">
        ...
        <div class="border-t border-[#F4F4F5] pt-6">
          ...
          <ol class="space-y-3">
            <li
              v-for="(step, idx) in selectedArticle.steps"
              :key="idx"
            ...
            >
              ...
              <p class="text-[14px] text-[#18181B] font-medium leading-relaxed">
                {{ step }}
              </p>
            </li>
          </ol>
        ...
        <div class="border-t border-[#F4F4F5] mt-8 pt-6 flex items-center justify-between">
          ...
          <div class="flex gap-2">
            <button
              class="flex items-center gap-1.5 px-3 py-1.5 rounded-[8px] border border-[#E4E4E7] text-[12px] font-semibold text-[#18181B] hover:bg-[#FAFAFA] transition-colors"
              @click="emit('notify', { message: 'Gracias por tu feedback', detail: 'Nos alegra que este artículo te haya sido útil.' })"
            >
              <span class="material-symbols-outlined text-[16px] text-emerald-500">thumb_up</span> Sí
            </button>
            <button
              class="flex items-center gap-1.5 px-3 py-1.5 rounded-[8px] border border-[#E4E4E7] text-[12px] font-semibold text-[#18181B] hover:bg-[#FAFAFA] transition-colors"
              @click="emit('notify', { message: 'Gracias por tu feedback', detail: 'Trabajaremos en mejorar este artículo.' })"
            >
    ...
    <div v-else>
      ...
      >
        <div class="w-8 h-8 border-4 border-[#2563EB] border-t-transparent rounded-full animate-spin" />
      </div>
      ...
      >
        ...
        >
          ...
          <h3 class="text-[14px] font-bold text-[#18181B] mb-1.5 group-hover:text-[#2563EB] transition-colors leading-snug">
            {{ article.title }}
          </h3>
          <p class="text-[12px] text-[#71717A] leading-relaxed line-clamp-2">
            {{ article.summary }}
          </p>
      ...
      <!-- Empty State -->
      <div
        v-if="!helpStore.isLoading && filteredArticles.length === 0"
        class="text-center py-12"
      >
        <span class="material-symbols-outlined text-[40px] text-[#D4D4D8] mb-3">search_off</span>
        <p class="text-[14px] font-bold text-[#18181B] mb-1">
          Sin resultados
        </p>
        <p class="text-[12px] text-[#71717A]">
          No encontramos artículos que coincidan con tu búsqueda. Intenta con otros términos.
        </p>
      </div>
      ...
      <div class="mb-8">
        <div class="flex items-center gap-2.5 mb-5">
          ...
          <h2 class="text-[18px] font-extrabold tracking-tight text-[#18181B]">
      ...
      <div class="bg-gradient-to-r from-[#2563EB] to-[#1d4ed8] rounded-[16px] p-6 sm:p-8 text-white shadow-lg">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div class="flex-1">
            <h3 class="text-[16px] font-extrabold tracking-tight mb-1">
              ¿No encontraste lo que buscabas?
            </h3>
            <p class="text-[13px] font-medium text-blue-100 leading-relaxed">
              Nuestro equipo de soporte está disponible de lunes a viernes de 8:00 a.m. a 6:00 p.m. (COT). Tiempo de respuesta promedio: 2 horas.
            </p>
          </div>
          <button
      ...
      <div class="relative bg-white rounded-[18px] shadow-[0_8px_60px_rgba(0,0,0,0.18)] w-full max-w-[500px] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="px-6 py-4 border-t border-[#F4F4F5] bg-[#FAFAFA] flex items-center justify-between gap-3">
          <p class="text-[11px] text-[#A1A1AA]">
            Tiempo de respuesta: <span class="font-semibold text-[#18181B]">≤ 4 horas hábiles</span>
          </p>
          <div class="flex gap-2">
            <button
              class="px-4 py-2 rounded-[8px] border border-[#E4E4E7] text-[12px] font-semibold text-[#71717A] hover:bg-white transition-colors"
              @click="closeTicketModal"
            >
              Cancelar
            </button>
