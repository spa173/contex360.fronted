...
function handleApproveEntry() {
  emit('notify', { message: 'Asiento aprobado', detail: 'Se generó el asiento contable por comisión bancaria.' })
}
</script>
  ...
  <section v-if="isActive" class="animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-8">
      ...
      <button class="flex items-center gap-2 px-3.5 py-2.5 bg-[#18181B] text-white rounded-[10px] hover:bg-[#27272A] text-[13px] font-semibold">
        <span class="material-symbols-outlined text-[18px]">auto_awesome</span>Conciliar con IA
      </button>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div class="bg-white border border-[#E4E4E7] rounded-[14px] p-5"><p class="text-[11px] font-semibold text-[#A1A1AA] uppercase tracking-wider mb-1">Partidas pendientes</p><p class="text-[22px] font-bold text-amber-700 tracking-[-0.02em] font-mono">12</p><p class="text-[11px] text-amber-700 font-semibold mt-1">Requieren revisión</p></div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            ...
            <tbody class="text-[12px] divide-y divide-[#F4F4F5]">
              ...
              <tr class="bg-rose-50/50 hover:bg-rose-50">
          ...
          <div class="p-4">
            ...
            <div class="bg-[#FAFAFA] rounded-[10px] p-3 border border-[#F4F4F5] mb-3">
              ...
              <div class="space-y-1 font-mono text-[11px]">
                <div class="flex justify-between"><span class="text-[#71717A]">6502 Gastos banc.</span><span class="text-emerald-700">{{ formatCurrency(1500) }}</span></div>
                <div class="flex justify-between pl-3"><span class="text-[#71717A]">1001 Banco</span><span class="text-rose-600">{{ formatCurrency(1500) }}</span></div>
              </div>
            </div>
            <div class="flex gap-2">
              <button @click="handleApproveEntry" class="flex-1 py-2 bg-[#2563EB] text-white rounded-[8px] text-[12px] font-semibold hover:bg-[#1D4ED8]">Aprobar</button>
              <button class="flex-1 py-2 border border-[#E4E4E7] text-[#71717A] rounded-[8px] text-[12px] font-semibold hover:bg-[#FAFAFA]">Ignorar</button>
            </div>
          </div>
        </div>
      </div>
