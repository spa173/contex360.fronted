  ...
  >
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-8">
      <div>
        ...
        <p class="text-[14px] text-[#71717A]">
          Cuenta 1001-00 · Banco Bogotá Cta Cte
        </p>
      </div>
      <button
        :disabled="isReconciling"
        class="flex items-center gap-2 px-3.5 py-2.5 bg-[#18181B] text-white rounded-[10px] hover:bg-[#27272A] disabled:opacity-50 text-[13px] font-semibold transition-all"
        @click="handleReconcileAi"
      >
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <div class="bg-white border border-[#E4E4E7] rounded-[14px] p-5">
        ...
        <p
          v-if="accounting.isLoading"
          class="text-[11px] text-[#A1A1AA] mt-1"
        >
          Cargando...
        </p>
        <p
          v-else
          class="text-[11px] text-emerald-700 font-semibold mt-1"
        >
          Saldo actual del tenant
        </p>
      </div>
      <div class="bg-white border border-[#E4E4E7] rounded-[14px] p-5">
        <p class="text-[11px] font-semibold text-[#A1A1AA] uppercase tracking-wider mb-1">
        ...
        <p class="text-[11px] text-[#71717A] mt-1">
          {{ reconciledCount }} conciliados auto
        </p>
      </div>
      <div class="bg-white border border-[#E4E4E7] rounded-[14px] p-5">
        <p class="text-[11px] font-semibold text-[#A1A1AA] uppercase tracking-wider mb-1">
      ...
      <div class="bg-white border border-[#E4E4E7] rounded-[14px] p-5">
        ...
        <p class="text-[22px] font-bold text-emerald-700 tracking-[-0.02em] font-mono">
          {{ formatCurrency(0) }}
        </p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-4">
            ...
            <tbody class="text-[12px] divide-y divide-[#F4F4F5]">
              ...
              >
                ...
                <td class="px-4 py-3 text-[#71717A]">
                  {{ new Date(latestEntry.entryAt || latestEntry.createdAt).toLocaleDateString() }}
                </td>
                <td class="px-4 py-3 font-mono text-[#A1A1AA] text-[11px]">
                  {{ latestEntry.referenceId || latestEntry.id }}
                </td>
                <td class="px-4 py-3 font-semibold text-[#18181B]">
                  {{ latestEntry.description }}
                </td>
                <td class="px-4 py-3 text-right font-mono text-rose-600 font-semibold">
                  {{ formatCurrency(latestEntry.amount) }}
                </td>
                <td class="px-4 py-3 text-right font-mono font-semibold text-[#18181B]">
                  {{ formatCurrency(bankBalance) }}
                </td>
              </tr>
              ...
              <tr
                ...
                class="bg-rose-50/50 hover:bg-rose-50"
              >
                <td class="px-4 py-3 text-center">
                  <span class="material-symbols-outlined text-[18px] text-amber-600">help</span>
                </td>
                <td class="px-4 py-3 text-[#71717A]">
                  Ayer
                </td>
                ...
                <td class="px-4 py-3 text-right font-mono font-semibold text-[#18181B]">
                  {{ formatCurrency(1170100) }}
                </td>
              </tr>
              <tr
                v-for="entry in accounting.tenantLedgerEntries"
                :key="entry.id"
                class="hover:bg-[#FAFAFA]"
              >
                <td class="px-4 py-3 text-center">
                ...
                <td class="px-4 py-3 text-[#71717A]">
                  {{ new Date(entry.entryAt || entry.createdAt).toLocaleDateString() }}
                </td>
                <td class="px-4 py-3 font-mono text-[#A1A1AA] text-[11px]">
                ...
                <td class="px-4 py-3 text-right font-mono text-[#18181B] font-semibold">
                  {{ formatCurrency(entry.amount) }}
                </td>
                <td class="px-4 py-3 text-right font-mono font-semibold text-[#71717A]">
                  —
                </td>
              </tr>
            </tbody>
      ...
      <div class="space-y-3">
        ...
        <div
          v-if="showAiDiscrepancy"
          class="bg-white border border-[#E4E4E7] rounded-[14px] overflow-hidden transition-all"
        >
          <div class="px-4 py-3 bg-amber-50 border-b border-amber-100 flex items-center gap-2">
            ...
            <p class="text-[12px] font-bold text-amber-700 uppercase tracking-wider">
              Discrepancia detectada
            </p>
          </div>
          <div class="p-4">
            <p class="text-[12px] text-[#71717A] leading-[1.5] mb-3">
              Cargo bancario <span class="font-mono font-semibold text-[#18181B]">TRX-8920</span> sin asiento contable.
            </p>
            <div class="bg-[#FAFAFA] rounded-[10px] p-3 border border-[#F4F4F5] mb-3">
              <p class="text-[10px] font-bold text-[#A1A1AA] uppercase tracking-wider mb-2">
                Asiento sugerido
              </p>
              <div class="space-y-1 font-mono text-[11px]">
                <div class="flex justify-between">
                  <span class="text-[#71717A]">530505 Gastos banc.</span><span class="text-emerald-700">{{ formatCurrency(1500) }}</span>
                </div>
                <div class="flex justify-between pl-3">
                  <span class="text-[#71717A]">111005 Banco</span><span class="text-rose-600">{{ formatCurrency(1500) }}</span>
                </div>
              </div>
            </div>
            <div class="flex gap-2">
              <button
                class="flex-1 py-2 bg-[#2563EB] text-white rounded-[8px] text-[12px] font-semibold hover:bg-[#1D4ED8] transition-colors"
                @click="handleApproveEntry"
              >
                Aprobar
              </button>
              <button
                class="flex-1 py-2 border border-[#E4E4E7] text-[#71717A] rounded-[8px] text-[12px] font-semibold hover:bg-[#FAFAFA] transition-colors"
                @click="handleIgnoreEntry"
              >
                Ignorar
              </button>
            </div>
        ...
        <div
          v-else
          class="bg-[#F8FAFC] border border-[#E2E8F0] rounded-[14px] p-5 text-center"
        >
          <div class="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600 mx-auto mb-2">
            <span class="material-symbols-outlined text-[20px]">check</span>
          </div>
          <h4 class="text-[13px] font-bold text-[#18181B] mb-1">
            Todo al día
          </h4>
          <p class="text-[12px] text-[#71717A]">
            No hay discrepancias pendientes por revisar.
          </p>
        </div>
      </div>
    </div>
