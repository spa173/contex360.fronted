  ...
  <div class="min-h-screen bg-white text-[#18181B] font-['Inter'] relative overflow-x-hidden">
    ...
    <section class="relative pt-14 pb-10">
      <div class="max-w-3xl mx-auto px-6 lg:px-8">
        <p class="text-[11px] uppercase tracking-[0.2em] font-bold text-[#2563EB] mb-4">Operaciones</p>
    ...
    <section class="relative pb-2">
      <div class="max-w-3xl mx-auto px-6 lg:px-8 space-y-12">
        <article v-for="s in sections" :key="s.n">
          ...
          <p v-if="s.body" class="text-[14px] text-[#71717A] leading-[1.65] font-medium">
            {{ s.body }}
          </p>

          <div v-if="s.items" class="grid sm:grid-cols-3 gap-3 mt-4">
            <div v-for="item in s.items" :key="item.label" class="p-4 rounded-[10px] border border-[#E4E4E7] bg-white">
              <p class="text-[10px] font-bold text-[#18181B] uppercase tracking-wider mb-1">{{ item.label }}</p>
              <p class="text-[18px] font-bold text-[#2563EB]">{{ item.value }}</p>
            </div>
          </div>

          <ul v-if="s.list" class="space-y-2 text-[14px] text-[#71717A] leading-[1.55] font-medium mt-3">
          ...
          <ol v-if="s.steps" class="space-y-2 text-[14px] text-[#71717A] leading-[1.55] font-medium mt-3">
            <li v-for="(step, idx) in s.steps" :key="idx" class="flex gap-3">
