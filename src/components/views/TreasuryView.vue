  ...
  >
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-8">
      <div>
        ...
        <p class="text-[14px] text-[#71717A]">
          Gestión de liquidez, programación de pagos y flujo de caja.
        </p>
      </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div class="bg-white border border-[#E4E4E7] rounded-[14px] p-5">
        ...
        <p class="text-[22px] font-bold text-[#18181B] tracking-[-0.02em] font-mono">
          {{ treasury.cashFlowTrend?.projected?.length > 0 ? formatCurrency(treasury.cashFlowTrend.projected[treasury.cashFlowTrend.projected.length - 1].balance) : 'Activa' }}
        </p>
      </div>
      <div class="bg-white border border-[#E4E4E7] rounded-[14px] p-5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left min-w-[540px]">
            ...
            <tbody class="text-[13px] divide-y divide-[#F4F4F5]">
              <tr
                v-for="item in programmedPayments"
                :key="item.id"
              ...
              >
                ...
                <td class="px-5 py-3.5">
                  <span :class="['inline-flex px-2 py-0.5 rounded-md text-[11px] font-semibold uppercase', priorityClass(item.priority)]">{{ item.priority }}</span>
                </td>
                <td class="px-5 py-3.5 text-right font-mono font-semibold">
                  {{ formatCurrency(item.amount) }}
                </td>
                <td class="px-5 py-3.5">
                  <span class="flex items-center gap-1.5 text-[12px]"><span class="w-1.5 h-1.5 rounded-full bg-[#2563EB]" />{{ item.status }}</span>
                </td>
              </tr>
      ...
      <div class="space-y-3">
        ...
        <div
          ...
          class="bg-white border border-[#E4E4E7] rounded-[14px] p-4 transition-all"
        >
          <div class="flex items-center gap-2 mb-2">
            <span class="material-symbols-outlined text-[16px] text-[#2563EB]">lightbulb</span><p class="text-[11px] font-bold text-[#18181B] uppercase tracking-wider">
              Optimización
            </p>
          </div>
          <p class="text-[12px] text-[#71717A] leading-[1.5] mb-3">
            {{ treasury.aiInsights?.insight || `Mover el pago a <strong>${programmedPayments[0]?.vendorName}</strong> al día 25 para evitar déficit temporal proyectado.` }}
          </p>
          <div class="flex gap-2">
            <button
              class="flex-1 py-1.5 bg-[#2563EB] text-white rounded-[8px] text-[11px] font-semibold hover:bg-[#1D4ED8] transition-colors"
              @click="handleApplyInsight"
            >
              Aplicar
            </button>
            <button
              class="px-3 py-1.5 border border-[#E4E4E7] text-[#71717A] rounded-[8px] text-[11px] font-semibold hover:bg-[#FAFAFA] transition-colors"
              @click="handleIgnoreInsight"
            >
              Ignorar
            </button>
          </div>
        ...
        <div
          v-else
          class="bg-[#F8FAFC] border border-[#E2E8F0] rounded-[14px] p-4 text-center"
        >
          ...
          <p class="text-[11px] text-[#71717A] mt-0.5">
            Programación ajustada exitosamente.
          </p>
        </div>
      </div>
    </div>
    ...
    >
      <div class="bg-white w-[90%] max-w-[400px] rounded-[16px] shadow-2xl border border-[#E4E4E7] overflow-hidden animate-in zoom-in-95 duration-200">
        <div class="px-5 py-4 border-b border-[#F4F4F5] flex justify-between items-center">
          ...
          <button
            class="text-[#A1A1AA] hover:text-[#18181B] transition-colors"
        ...
        <div class="p-5 space-y-4">
          <div>
            ...
            <input
              v-model="newPaymentForm.vendorName"
              type="text"
              placeholder="Ej. Papelería SAS"
              class="w-full px-3 py-2 bg-[#FAFAFA] border border-[#E4E4E7] rounded-[8px] text-[13px] text-[#18181B] focus:outline-none focus:border-[#2563EB] focus:ring-1 focus:ring-[#2563EB] transition-all"
            >
          </div>
          <div>
            ...
            <input
              ...
              class="w-full px-3 py-2 bg-[#FAFAFA] border border-[#E4E4E7] rounded-[8px] text-[13px] text-[#18181B] focus:outline-none focus:border-[#2563EB] focus:ring-1 focus:ring-[#2563EB] transition-all font-mono"
            >
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                v-model="newPaymentForm.dueDate"
                type="date"
                class="w-full px-3 py-2 bg-[#FAFAFA] border border-[#E4E4E7] rounded-[8px] text-[13px] text-[#18181B] focus:outline-none focus:border-[#2563EB] focus:ring-1 focus:ring-[#2563EB] transition-all"
              >
            </div>
            <div>
              ...
              <select
                v-model="newPaymentForm.priority"
                class="w-full px-3 py-2 bg-[#FAFAFA] border border-[#E4E4E7] rounded-[8px] text-[13px] text-[#18181B] focus:outline-none focus:border-[#2563EB] focus:ring-1 focus:ring-[#2563EB] transition-all appearance-none"
              >
                <option value="Alta">
                  Alta
                </option>
        ...
        <div class="px-5 py-4 bg-[#FAFAFA] border-t border-[#F4F4F5] flex justify-end gap-2">
          <button
            class="px-4 py-2 border border-[#E4E4E7] bg-white text-[#71717A] rounded-[8px] text-[12px] font-semibold hover:bg-[#F4F4F5] transition-colors"
            @click="showPaymentModal = false"
          >
            Cancelar
          </button>
          <button
            :disabled="!newPaymentForm.vendorName || !newPaymentForm.amount || treasury.isSaving"
            class="flex items-center justify-center min-w-[120px] px-4 py-2 bg-[#18181B] text-white rounded-[8px] text-[12px] font-semibold hover:bg-[#27272A] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            @click="submitPayment"
          >
            <span
              v-if="treasury.isSaving"
