...
<template>
  <div class="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-slate-50/30 font-sans antialiased">

    <!-- Navbar mínimo -->
    <header class="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-emerald-100/60 px-6 h-14 flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <span class="font-bold text-gray-900 text-sm">Contex<span class="text-emerald-600">360</span></span>
      </div>
      <button
        @click="emit('back')"
        class="text-sm text-gray-400 hover:text-emerald-700 transition-colors flex items-center gap-1.5"
      >
        â† Volver al inicio
      </button>
    </header>
    ...
    <div class="max-w-xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-10">
        <div class="inline-flex items-center gap-2 bg-emerald-100 text-emerald-800 rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider mb-5">
      ...
      <!-- Success -->
      <div v-if="successMessage" class="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 text-center mb-6">
        ...
        <button @click="successMessage = ''" class="mt-4 text-sm text-emerald-700 hover:text-emerald-800 font-medium transition-colors">
      ...
      <!-- Form -->
      <form v-if="!successMessage" @submit.prevent="handleSubmit" class="bg-white rounded-2xl border border-gray-100 shadow-sm shadow-emerald-100/50 p-8 space-y-5">

        <div class="space-y-1.5">
          <label class="block text-sm font-semibold text-gray-700">
            Nombre completo <span class="text-emerald-600">*</span>
          </label>
          <input
            v-model="form.nombre"
            type="text"
            ...
            required
            :disabled="isLoading"
            class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:border-transparent focus:bg-white transition-all disabled:opacity-50"
          />
        </div>

        <div class="space-y-1.5">
          <label class="block text-sm font-semibold text-gray-700">
            Empresa <span class="text-emerald-600">*</span>
          </label>
          <input
        ...
        <div class="space-y-1.5">
          <label class="block text-sm font-semibold text-gray-700">
            Correo electrónico <span class="text-emerald-600">*</span>
          </label>
          <input
            v-model="form.correo"
            type="email"
        ...
        <div class="space-y-1.5">
          ...
          <textarea
            ...
            :disabled="isLoading"
            class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:border-transparent focus:bg-white transition-all disabled:opacity-50 resize-none"
          />
        </div>
        ...
        <button
          type="submit"
          :disabled="isLoading"
          class="w-full py-4 rounded-xl bg-gradient-to-r from-emerald-700 to-emerald-600 hover:from-emerald-800 hover:to-emerald-700 text-white font-bold text-sm shadow-lg shadow-emerald-200 hover:shadow-emerald-300 transition-all disabled:opacity-60 disabled:cursor-not-allowed hover:-translate-y-0.5 active:translate-y-0"
        >
          {{ isLoading ? 'Enviando...' : 'Solicitar Demo gratis' }}
        </button>

        <p class="text-center text-xs text-gray-400">
          Al enviar aceptas nuestra <a href="#" class="text-emerald-700 hover:underline">política de privacidad</a>
        </p>
      </form>
