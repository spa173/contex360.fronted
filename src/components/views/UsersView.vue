...
function handleStatusToggle(user) {
  emit('notify', { message: 'Estado actualizado', detail: `Acceso de ${user.name} ${!user.active ? 'activado' : 'desactivado'}.` })
}

function handleApplyInsight(action) {
  emit('notify', { message: 'Insight aplicado', detail: `La acción "${action}" fue ejecutada.` })
}
</script>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
      <div class="bg-white border border-[#E4E4E7] rounded-[14px] overflow-hidden">
        <div class="px-5 py-4 border-b border-[#F4F4F5] flex items-center justify-between gap-3 flex-wrap">
          ...
          <select v-model="selectedRole" class="border border-[#E4E4E7] rounded-[8px] py-2 px-3 text-[12px] font-semibold text-[#71717A] outline-none bg-white">
            <option>Todos los roles</option><option>Administrador</option><option>Finanzas</option><option>Ventas</option>
          </select>
        </div>
