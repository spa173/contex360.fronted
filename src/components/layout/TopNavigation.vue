      ...
      <div class="flex items-center gap-1.5 ml-auto flex-shrink-0">
        ...
        >
          ...
          >
            ...
            <span
              v-if="unreadCount > 0"
              class="absolute top-1 right-1 w-2.5 h-2.5 rounded-full bg-[#2563EB] border-2 border-white"
            />
          </button>
          ...
          <div
            ...
            data-dropdown
            class="absolute right-0 top-full mt-1.5 w-[300px] bg-white border border-[#E4E4E7] rounded-[14px] shadow-[0_1px_2px_rgba(0,0,0,0.02),0_24px_60px_-20px_rgba(10,10,10,0.18)] overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200"
          >
            <div class="px-4 py-3 border-b border-[#F4F4F5] flex items-center justify-between">
              <div class="flex items-center gap-2">
                <h3 class="text-[13px] font-bold tracking-tight text-[#18181B]">
                  Notificaciones
                </h3>
                <span
                  v-if="unreadCount > 0"
                  class="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1.5 rounded-full bg-[#2563EB] text-white text-[10px] font-bold"
                >{{ unreadCount }}</span>
              </div>
              <button
                class="text-[11px] font-semibold text-[#71717A] hover:text-[#18181B]"
                @click="markAllAsRead"
              >
                Marcar todo leído
              </button>
            ...
            <div class="max-h-[380px] overflow-y-auto">
              ...
              >
                <span class="animate-spin w-6 h-6 border-2 border-[#18181B] border-t-transparent rounded-full mb-2" />
              ...
              >
                ...
                <p class="text-[12px] font-bold text-[#18181B]">
                  Sin alertas nuevas
                </p>
                ...
                >
                  <div class="flex items-start gap-2.5">
                    ...
                    <div class="flex-1 min-w-0">
                      <p class="text-[12px] font-semibold text-[#18181B] mb-0.5">
                        {{ notif.title }}
                      </p>
                      <p class="text-[11px] text-[#71717A] leading-snug">
                        {{ notif.description }}
                      </p>
                      <p class="text-[10px] text-[#A1A1AA] mt-1">
                        {{ notif.time }}
                      </p>
                    </div>
                  </div>
          ...
          >
            ...
            <div
              data-dropdown
              class="fixed top-[60px] left-3 right-3 z-[200] max-h-[calc(100dvh-5rem)] overflow-y-auto bg-white border border-[#E4E4E7] rounded-[14px] shadow-[0_1px_2px_rgba(0,0,0,0.02),0_24px_60px_-20px_rgba(10,10,10,0.18)] overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200"
            >
              <div class="px-4 py-3 border-b border-[#F4F4F5] flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <h3 class="text-[13px] font-bold tracking-tight text-[#18181B]">
        ...
        <button
          ...
          @click="emit('navigate', 'help-center')"
        >
          <span class="material-symbols-outlined text-[20px]">help</span>
        </button>

        <div class="w-px h-5 bg-[#E4E4E7] mx-0.5" />
        ...
        >
          ...
          >
            ...
            <div class="text-left hidden lg:block min-w-0">
              <p class="text-[12px] font-bold text-[#18181B] leading-tight">
                {{ (user?.name || 'Usuario').split(' ').slice(0,2).map((w,i)=>i===1?w[0]+'.':w).join(' ') }}
              </p>
              <p class="text-[10px] font-semibold text-[#A1A1AA] leading-tight uppercase tracking-wider">
                {{ activeMembership?.role || user?.title || 'Administrador' }}
              </p>
            </div>
          ...
          >
            ...
            <div class="px-4 py-3.5 flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-[14px] font-bold text-[#18181B] tracking-tight truncate leading-tight">
                  {{ user?.name || 'Usuario' }}
                </p>
                <p class="text-[12px] text-[#71717A] truncate leading-snug">
            ...
            <div
              v-if="activeTenant"
              class="mx-3 mb-1 px-3 py-2.5 rounded-[10px] bg-[#FAFAFA] border border-[#F4F4F5]"
            >
              ...
              <div class="flex items-center gap-2.5 justify-between">
                <div class="flex items-center gap-2.5 min-w-0">
                  ...
                  <div class="min-w-0">
                    <p class="text-[13px] font-bold text-[#18181B] truncate leading-none mb-1">
                      {{ activeTenant?.name || 'Andina Cargo SAS' }}
                    </p>
            ...
            <div class="py-1 border-t border-[#F4F4F5] mt-1">
              <button
                class="w-full flex items-center gap-3 px-4 py-2 hover:bg-[#FAFAFA] text-left"
                @click="emit('navigate', 'profile'); showAvatar = false"
              >
              ...
              <button
                class="w-full flex items-center gap-3 px-4 py-2 hover:bg-[#FAFAFA] text-left"
                @click="emit('navigate', 'two-factor'); showAvatar = false"
              >
              ...
              <button
                class="w-full flex items-center gap-3 px-4 py-2 hover:bg-[#FAFAFA] text-left"
                @click="emit('navigate', 'admin-console'); showAvatar = false"
              >
              ...
              <button
                ...
                class="w-full flex items-center gap-3 px-4 py-2 hover:bg-amber-50 text-left border-t border-amber-100/50"
                @click="emit('open-admin-panel'); showAvatar = false"
              >
            ...
            <div class="py-1 border-t border-[#F4F4F5]">
              <button
                class="w-full flex items-center gap-3 px-4 py-2 hover:bg-[#FAFAFA] text-left"
                @click="emit('navigate', 'help-center'); showAvatar = false"
              >
              ...
              <button
                class="w-full flex items-center gap-3 px-4 py-2 hover:bg-[#FAFAFA] text-left"
                @click="showShortcuts = true; showAvatar = false"
              >
            ...
            <div class="py-1 border-t border-[#F4F4F5]">
              <button
                class="w-full flex items-center gap-3 px-4 py-2 hover:bg-rose-50 text-left"
                @click="emit('logout'); showAvatar = false"
              >
    ...
    >
      ...
      <div class="relative bg-white rounded-[18px] shadow-[0_8px_60px_rgba(0,0,0,0.18)] w-full max-w-[480px] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-[#F4F4F5]">
          ...
          <button
            class="w-7 h-7 rounded-[8px] hover:bg-[#F4F4F5] flex items-center justify-center text-[#A1A1AA] hover:text-[#18181B] transition-colors"
            @click="showShortcuts = false"
          >
        ...
        <div class="overflow-y-auto max-h-[70vh] px-6 py-4 space-y-5">
          <div
            ...
            :key="group.group"
          >
            <p class="text-[10px] font-extrabold uppercase tracking-widest text-[#A1A1AA] mb-2.5">
              {{ group.group }}
            </p>
            <div class="space-y-1">
              <div
        ...
        <div class="px-6 py-3 border-t border-[#F4F4F5] bg-[#FAFAFA] text-center">
          <p class="text-[11px] text-[#A1A1AA] font-medium">
            En Mac usa <kbd class="inline-flex items-center px-1 text-[10px] font-mono bg-white border border-[#E4E4E7] rounded">⌘</kbd> · En Windows/Linux usa <kbd class="inline-flex items-center px-1 text-[10px] font-mono bg-white border border-[#E4E4E7] rounded">Ctrl</kbd>
          </p>
        </div>
      </div>
    </div>
