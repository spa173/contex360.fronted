...
<template>
  <div class="space-y-8">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-bold text-white mb-1">Alertas de Brecha de Seguridad</h2>
        <p class="text-sm text-slate-400">Detección y respuesta ante incidentes según Ley 1581</p>
      </div>
    ...
    <div class="grid grid-cols-1 gap-6">
      ...
      >
        <div class="flex items-start justify-between mb-4">
          <div class="flex items-center gap-3">
            <div :class="['w-10 h-10 rounded-lg flex items-center justify-center', alert.severity === 'critical' ? 'bg-red-500/10' : 'bg-amber-500/10']">
              <ShieldAlert class="w-6 h-6" :class="alert.severity === 'critical' ? 'text-red-500' : 'text-amber-500'" />
            </div>
            <div>
              <h4 class="text-white font-bold">{{ alert.action }}</h4>
              <p class="text-xs text-slate-500">{{ formatDate(alert.occurredAt) }}</p>
            </div>
          </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-white/5">
          ...
          >
            <Send v-if="notifyingId !== alert.id" class="w-4 h-4" />
            <RotateCw v-else class="w-4 h-4 animate-spin" />
            {{ notifyingId === alert.id ? 'Notificando...' : 'Notificar Administradores' }}
          </button>
          <div v-else class="flex items-center gap-2 text-emerald-500 bg-emerald-500/10 px-4 py-2 rounded-lg border border-emerald-500/20">
             <CheckCircle class="w-4 h-4" />
             <span class="text-xs font-bold uppercase">Notificado el {{ formatDate(alert.notifiedAt) }}</span>
          </div>
        </div>
      </div>
      <div v-if="!breachAlerts.length" class="bg-[#14141b] border border-white/5 rounded-2xl p-20 text-center">
         <ShieldCheck class="w-16 h-16 text-emerald-500/20 mx-auto mb-4" />
         <h3 class="text-xl font-bold text-white mb-2">No se han detectado brechas</h3>
         <p class="text-slate-400">El sistema de monitoreo en tiempo real no ha identificado incidentes de seguridad.</p>
      </div>
    </div>
  </div>
</template>
