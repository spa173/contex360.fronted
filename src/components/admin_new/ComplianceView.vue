  ...
  <div class="space-y-8">
    <div>
      <h2 class="text-2xl font-bold text-white mb-1">
        Cumplimiento ISO 27001 / Ley 1581
      </h2>
      <p class="text-sm text-slate-400">
        Control de controles técnicos y evidencias de auditoría
      </p>
    </div>
    ...
    <div
      v-if="accessReview"
      class="bg-[#111827] border border-white/5 rounded-xl p-8"
    >
      <div class="flex items-start justify-between mb-8">
        <div>
          ...
          <p class="text-sm text-slate-400">
            Control preventivo para mitigar riesgos de privilegios excesivos
          </p>
        </div>
        <button 
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="space-y-4">
          <div
            v-for="finding in accessReview.findings"
            :key="finding.label"
            class="bg-white/5 rounded-lg p-4 flex items-center justify-between"
          >
            <div class="flex items-center gap-3">
              <div :class="['w-2 h-2 rounded-full', finding.severity === 'high' ? 'bg-red-500' : 'bg-amber-500']" />
        ...
        <div class="bg-black/20 rounded-xl p-4 overflow-hidden border border-white/5">
          ...
          <div class="space-y-3">
            <div
              v-for="run in accessReview.recentRuns"
              :key="run.id"
              class="flex items-center justify-between text-xs py-2 border-b border-white/5 last:border-0"
            >
