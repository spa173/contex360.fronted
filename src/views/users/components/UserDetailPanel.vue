  ...
  <aside class="bg-[#131926] border border-slate-800/60 rounded-2xl p-0 shadow-2xl overflow-hidden user-detail-panel transition-all duration-300">
    ...
    <div class="px-8 py-7 bg-gradient-to-br from-slate-800/20 to-transparent border-b border-slate-800/40">
      <div class="flex justify-between items-start mb-4">
        <div>
          <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-500 mb-2">
            Perfil de Usuario
          </p>
          <h3 class="text-2xl font-black text-white tracking-tighter">
            {{ row.user.name }}
          </h3>
          <p class="text-sm font-mono text-slate-500 tracking-tight">
            {{ row.user.email }}
          </p>
        </div>
        <div
    ...
    <div class="p-8 space-y-10">
      ...
      <section>
        <div class="flex items-center gap-2 mb-5">
          ...
          <h4 class="text-xs font-bold uppercase tracking-[0.15em] text-slate-400">
        ...
        <div class="grid grid-cols-2 gap-px bg-slate-800/20 rounded-xl border border-slate-800/40 overflow-hidden">
          <div
            ...
            :key="label"
            class="bg-[#131926] p-4 flex flex-col gap-1"
          >
      ...
      <section>
        ...
        <form
          class="mt-6 p-4 bg-slate-800/10 rounded-xl border border-dashed border-slate-800/60 space-y-4"
          @submit.prevent="handleMembershipSubmit"
        >
          <div class="grid grid-cols-1 gap-3">
            <select
              v-model="membershipForm.tenantId"
              class="bg-slate-900 border-slate-800 text-xs text-slate-300 rounded-lg focus:ring-emerald-500"
            >
              <option value="">
                Seleccionar empresa...
              </option>
          ...
          <button
            class="w-full py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-bold rounded-lg transition-all"
            type="submit"
          >
            Asignar Nuevo Rol
          </button>
      ...
      <section>
        <div class="flex items-center justify-between mb-5">
          ...
          <button
            class="text-[9px] font-bold text-slate-500 hover:text-slate-300 uppercase tracking-widest"
            @click="$emit('revokeSessions', row.user.id)"
          >
            Revocar Todas
          </button>
        </div>
        ...
        >
          ...
          >
            <div class="space-y-1">
              <p class="text-[10px] font-mono font-bold text-slate-300 leading-none">
                {{ formatSessionLabel(session) }}
              </p>
              <p class="text-[9px] text-slate-500 leading-none tracking-tight">
                {{ session.location }} · {{ formatDate(session.lastSeenAt) }}
              </p>
            </div>
            <button
        ...
        <div
          v-else
          class="py-10 text-center bg-slate-800/5 rounded-2xl border border-dashed border-slate-800/40"
        >
        ...
        <div class="p-4 bg-rose-500/[0.03] border border-rose-500/10 rounded-xl space-y-4">
          <div class="grid grid-cols-1 gap-4">
            <div class="space-y-1.5">
              ...
              <input
                v-model="offboardingForm.date"
                type="date"
                class="w-full bg-slate-900 border-slate-800 text-xs text-slate-300 rounded-lg"
              >
            </div>
            <div class="space-y-1.5">
              <span class="text-[9px] uppercase font-bold text-slate-500 tracking-wider">Reasignar Auditoría a</span>
              <select
                v-model="offboardingForm.reassignToUserId"
                class="w-full bg-slate-900 border-slate-800 text-xs text-slate-300 rounded-lg"
              >
                <option value="">
                  Sin reasignación
                ...
                <option
                  v-for="userRow in userRows"
                  :key="`off-${userRow.user.id}`"
                  :value="userRow.user.id"
                >
                  {{ userRow.user.name }}
                </option>
              </select>
          ...
          <button
            ...
            @click="handleScheduleDeactivation"
          >
